...
#include <openssl/evp.h> // libssl-dev
...
const char* basename(const char *fname) noexcept
{
	const char *s = fname;
	while(0 != *s) ++s;
...
int err_evp(const char* msg) { fprintf(stderr, "EVP Error: %s\n", msg); return ERR_CHECKSUM; }

int getchecksum(FILE *f, unsigned char *checksum, HEADER *header=0)
{
	unsigned char buf[BUFSIZE];
	HEADER local_header_buf; // Use a local buffer if header isn't passed
	HEADER *hdr_ptr;         // Pointer to the header data being checksummed
	int ret_val = 0;         // Return value, default 0 (success)

	// --- Prepare header data ---
	if (header == 0) // Read header from file if not provided
	{
		if (0 != fseek(f, 0, SEEK_SET))
			return err_fileseek(0);

		clearerr(f);
		if (fread(&local_header_buf, 1, sizeof(HEADER), f) < sizeof(HEADER))
		{
			if (ferror(f)) return err_fileread();
			return err_header();
		}
		hdr_ptr = &local_header_buf;
	}
	else // Use header provided as argument
	{
		// Ensure file pointer is positioned after the header
		if (0 != fseek(f, sizeof(HEADER), SEEK_SET))
			return err_fileseek(sizeof(HEADER));

		// Copy provided header to local buffer ONLY if modification is needed
		// Since we modify bootorder/bootstatus, we need a local copy
		// or modify the passed 'header' directly if that's acceptable.
		// Let's assume modifying a local copy is safer.
		memcpy(&local_header_buf, header, sizeof(HEADER));
		hdr_ptr = &local_header_buf;
	}

	// Modify fields required for checksum calculation *on the buffer/copy*
	hdr_ptr->bootorder = 0;
	hdr_ptr->bootstatus = 0xffffffff;

	// --- MD5 calculation using EVP ---
	EVP_MD_CTX *mdctx = NULL;
	unsigned int md5_digest_len = EVP_MD_size(EVP_md5()); // Should be CHECKSUM_SIZE (16)

	// Check if CHECKSUM_SIZE matches expected MD5 size
	if (md5_digest_len != CHECKSUM_SIZE) {
		fprintf(stderr, "Error: CHECKSUM_SIZE (%d) != EVP_MD_size(EVP_md5()) (%u)\n",
		        CHECKSUM_SIZE, md5_digest_len);
		return -4; // Indicate a mismatch error
	}
	...
	if (mdctx == NULL) {
		ret_val = err_evp("Failed to create context (EVP_MD_CTX_new)");
		goto cleanup; // Use goto for centralized cleanup
	}

	if (1 != EVP_DigestInit_ex(mdctx, EVP_md5(), NULL)) {
		ret_val = err_evp("Failed to initialize digest (EVP_DigestInit_ex)");
		goto cleanup;
	}

	// 2. Update with header data (excluding the checksum field itself)
	if (1 != EVP_DigestUpdate(mdctx, hdr_ptr, sizeof(HEADER) - CHECKSUM_SIZE)) {
		ret_val = err_evp("Failed to update digest with header (EVP_DigestUpdate)");
		goto cleanup;
	}


	// 3. Update with rest of the file data
	clearerr(f);
	for (;;)
	{
		int r = fread(buf, 1, sizeof(buf), f);
		if (r < 0) // Check for read error explicitly
        {
            if (ferror(f)) {
				ret_val = err_fileread();
				goto cleanup;
			}
            // Should not happen if r < 0 unless ferror is set? Treat as EOF for safety.
             break;
        }
        if (r == 0) // End of file
        {
            if (feof(f)) break; // Normal EOF
            // If r==0 but not EOF and not ferror, something is wrong, but proceed cautiously
            // or treat as error. Let's assume it means EOF here.
            break;
        }


		// Update digest with the chunk read
		if (1 != EVP_DigestUpdate(mdctx, buf, r)) {
			ret_val = err_evp("Failed to update digest with file data (EVP_DigestUpdate)");
			goto cleanup;
		}
	}

	// 4. Finalize the hash
	// Note: EVP_DigestFinal_ex modifies md5_digest_len to the actual bytes written
	if (1 != EVP_DigestFinal_ex(mdctx, checksum, &md5_digest_len)) {
		ret_val = err_evp("Failed to finalize digest (EVP_DigestFinal_ex)");
		goto cleanup;
	}

	// --- Post-processing ---
	// XOR with magic bytes (original logic)
	for (int n = 0; n < CHECKSUM_SIZE; ++n) {
		checksum[n] ^= CHECKSUM_MAGIC[n];
	}

cleanup:
	// 5. Free the context *always* if it was allocated
	if (mdctx != NULL) {
		EVP_MD_CTX_free(mdctx);
	}

	return ret_val; // Return 0 on success, error code otherwise
}
...
int updatesquashfs(FILE *f, int offset)
{
    ...
    printf("ERROR: bad squashfs compression, require gzip compression\n");
  	return ERR_PARTITION;
	default:
		printf("ERROR: bad squashfs compression: %d\n", buf.compression);
		return ERR_PARTITION;
	}
